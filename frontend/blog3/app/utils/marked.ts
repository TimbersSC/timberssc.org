...
export const markedExtended = (content: string) => {
  ...
  // Check footnote
  function checkFootnote(token: { type: string; text: string; toDelete: boolean; }) {
  ...
  function checkReference(token: { type: string; text: string | marked.Token[]; tokens: any; }) {
    if (token.type === 'paragraph' || token.type === 'text') {
      token.text = token.text.replace(referenceTest, (ref: any, value: string) => {
  ...
  function visit(tokens: typeof marked, fn: { (token: any): void; (token: any): void; (arg0: any): void; }) {
